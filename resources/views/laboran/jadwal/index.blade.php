    ...
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title"><i class="fa fa-clock-o"></i>&nbsp;Manajemen Data Jadwal Praktikum</h3>
                <div class="pull-right">
                    <a href="{{ route('laboran.jadwal.add') }}" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-plus"></i>&nbsp; Tambah Data Jadwal Praktikum</a>
                </div>
            </div>
            <div class="box-body">
                <div class="row">
                    ...
                    <table class="table table-striped table-bordered" id="table" style="width:100%;">
                        <thead>
                            <tr>
                                ...
                                <th>Program Studi</th>
                                <th>Jumlah Sesi</th>
                                <th style="text-align: center">Sesi Ditambahkan</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            @foreach ($jadwals as $jadwal)
                                <tr>
                                    <td> {{ $no++ }} </td>
                                    <td> {{ $jadwal->tanggal_praktikum }} </td>
                                    <td> {{ $jadwal->nama_mata_kuliah }} </td>
                                    <td> {{ $jadwal->nama_prodi }} </td>
                                    <td> {{ $jadwal->jumlah_sesi }} Sesi </td>
                                    <td style="text-align: center">
                                        <a href="{{ route('laboran.sesi',[$jadwal->id]) }}" class="btn btn-success btn-sm btn-flat">
                                            {{ $jadwal->sesis->count() }}
                                        </a>
                                    </td>
                                          <td style="display:inline-block !important;">
                                            <table>
                                                <tr>
                                                    <td>
                                                       <a href="{{ route('laboran.jadwal.edit',[$jadwal->id]) }}" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-edit"></i>&nbsp; Edit</a>
                                                    </td>
                                                    <td>
                                                       <form action="{{ route('laboran.jadwal.delete',[$jadwal->id]) }}" method="POST">
            ...
            {{--  <div class="modal fade modal-danger" id="modaldelete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action=" {{ route('laboran.jadwal.delete',[$indikator->id]) }}" method="POST">
                            {{ csrf_field() }} {{ method_field('DELETE') }}

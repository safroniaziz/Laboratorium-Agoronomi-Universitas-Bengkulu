...
class MahasiswaKewirausahaanDashboardController extends Controller
{
    ...
    public function dashboard(){
        $prodi = Prodi::select('id')->where('kode_prodi',"ALL")->first();
        if (empty($prodi) || $prodi == "" || $prodi == null) {
            $matkuls = [];
        }else {
            $matkuls = MataKuliah::where('prodi_id',$prodi->id)->get();
        }
        return view('mahasiswa_kewirausahaan.dashboard',[
            'matkuls'  =>  $matkuls
        ]);
    }
    ...
    public function matkulDetail($matkul_id){
        ...
        return view('mahasiswa_kewirausahaan/matkul_detail',[
            'matkul' => $matkul,
            'matkul_id' => $matkul_id,
        ]);
    }
